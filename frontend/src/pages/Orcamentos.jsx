...
import { 
  ...
  FileText, Calendar, XCircle, Printer
} from 'lucide-react'
import api from '../services/api'

/**
 * GESTÃO DE ORÇAMENTOS - SANFLAUZIN
 * CORREÇÕES: 
 * 1. Sincronização de estoque na aprovação.
 * 2. Estabilização da rota de deleção.
 * 3. Atalho de impressão direta.
 */
export function Orcamentos() {
  ...
  // --- LÓGICA DE GESTÃO (COM ATUALIZAÇÃO DE ESTOQUE) ---

  const updateQuoteStatus = async (id, newStatus, confirmMsg) => {
    if (!window.confirm(confirmMsg)) return
    
    try {
      // O backend deve tratar a lógica de tirar/devolver ao estoque baseado no status
      await api.put(`/quotes/${id}`, { status: newStatus })
      
      alert(`Orçamento ${newStatus === 'APROVADO' ? 'aprovado e estoque atualizado' : 'rejeitado'}!`)
      fetchData() // Recarrega para refletir as mudanças
    } catch (err) {
      console.error(`Erro ao atualizar para ${newStatus}:`, err)
      const errorMsg = err.response?.data?.error || "Erro na comunicação com o servidor."
      alert(`Falha na operação: ${errorMsg}`)
    }
  }

  const handleApprove = (id) => 
    updateQuoteStatus(id, 'APROVADO', "Deseja aprovar? Isso confirmará a reserva definitiva dos materiais no estoque.")

  const handleReject = (id) => 
    updateQuoteStatus(id, 'REPROVADO', "Deseja reprovar? Os materiais reservados ficarão disponíveis para outras noivas.")

  const handleDelete = async (id) => {
    if (!window.confirm("ATENÇÃO: Excluir permanentemente este registro? Isso não afetará o estoque caso já tenha sido aprovado.")) return
    try {
      // Rota garantida: /quotes/:id
      await api.delete(`/quotes/${id}`)
      setQuotes(prev => prev.filter(q => q.id !== id))
      alert("Registro removido com sucesso.")
    } catch (err) {
      console.error("Erro ao excluir:", err)
      alert("Erro ao excluir. O registro pode já ter sido removido ou há um erro de conexão.")
    }
  }

  const handlePrint = (id) => {
    const printUrl = `${api.defaults.baseURL}/quotes/${id}/print`
    ...
    <div className="space-y-6 p-2">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-secondary-100 flex flex-col md:flex-row justify-between items-center gap-6">
        <div>
          <h1 className="text-2xl font-black text-secondary-900 flex items-center gap-2 italic uppercase tracking-tighter">
            <Calculator className="text-indigo-600" size={28} /> Gestão de Propostas
          </h1>
          <p className="text-[10px] font-bold text-secondary-400 uppercase tracking-widest mt-1">
            Reserva de estoque e fechamento de contratos
          </p>
        </div>
        ...
        <div className="flex flex-1 max-w-xl w-full gap-2">
          <div className="relative flex-1">
            ...
            <input 
              type="text" 
              placeholder="Buscar noiva ou orçamento..." 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-6 rounded-[2rem] border border-secondary-100 flex items-center justify-between shadow-sm">
          <div>
            <p className="text-[9px] font-black text-secondary-400 uppercase tracking-widest">Pipeline Financeiro (Filtro)</p>
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredQuotes.length > 0 ? (
            ...
            <div key={quote.id} className="bg-white p-6 rounded-[2.5rem] border-2 border-secondary-50 hover:shadow-xl hover:border-indigo-100 transition-all flex flex-col md:flex-row items-center gap-6 group">
              ...
              <div className="flex-1 text-center md:text-left">
                <div className="flex items-center justify-center md:justify-start gap-2 mb-1">
                  <span className="text-[9px] font-black bg-secondary-100 text-secondary-500 px-2 py-0.5 rounded-md uppercase">Ref: {quote.id?.substring(0,8)}</span>
                ...
                <div className="flex items-center justify-center md:justify-start gap-4 mt-2">
                   <div className="flex items-center gap-1 text-secondary-400 font-bold text-[10px] uppercase">
                      <Calendar size={12}/> Gerado em: {quote.createdAt ? new Date(quote.createdAt).toLocaleDateString('pt-BR') : '--/--/--'}
                   </div>
                   <div className="flex items-center gap-1 text-indigo-500 font-bold text-[10px] uppercase">
                      <ShoppingBag size={12}/> {quote.items?.length || 0} Materiais vinculados
                   </div>
                </div>
              </div>
              ...
              <div className="px-10 border-y-2 md:border-y-0 md:border-x-2 border-secondary-50 py-4 md:py-0 text-center min-w-[200px]">
                <p className="text-[9px] font-black text-secondary-400 uppercase tracking-widest mb-1">Total da Proposta</p>
              ...
              <div className="flex flex-wrap justify-center gap-2">
                ...
                <button 
                  ...
                  className="p-4 bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-2xl transition-all active:scale-90"
                  title="Imprimir PDF"
                >
                  <Printer size={22}/>
                </button>
                ...
                {quote.status === 'PENDENTE' && (
                  <>
                    <button 
                      ...
                      title="Aprovar e Dar Baixa no Estoque"
                    >
          ...
          <div className="bg-white rounded-[3rem] p-20 text-center border-2 border-dashed border-secondary-100 shadow-inner">
            ...
            <p className="text-secondary-400 font-black uppercase text-xs tracking-widest">Nenhum orçamento encontrado</p>
          </div>
        )}
      </div>
    </div>
